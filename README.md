...
* 7_Deep_learning.ipynb demonstrate deep learning on an image classification problem. The notebook foucses on reusing own or third party training results. It also demonstrates the value of data augmentation. Therefore, we train different CNNs from skretch and transfer pretrained models, with and without data augmentation, for solving one and the same classification problem.
