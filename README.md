...
* 6_Neural_Networks.ipynb applies a one layer artificial neural network (ANN) for classification on the Iris dataset and regression on the Boston dataset. The former uses softmax for activation the neurons and a cross-entropy cost function. The latter uses a linear activation (no activation) and a mean squared error cost function. Hence, it is actually the same as linear regression. Finally, to train non-linear regression model, we add a second layer with a non-linear activation to the regression ANN.
